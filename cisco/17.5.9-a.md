...
#### Scopo
Visualizza lo **stato dei protocolli di rete** attivi sulle interfacce, incluso il prefisso di rete completo. Mostra se IP routing è abilitato globalmente.
...
> [!NOTE]
> A differenza di `show ip interface brief`, questo comando mostra l'indirizzo IP **con il prefisso** (es. `/27`), utile per conoscere la dimensione della subnet.
...
#### Scopo
Visualizza le **sessioni utente attive** sul router: chi è connesso e da quale linea (console, VTY/Telnet/SSH, AUX).
...
| Campo | Valore | Significato |
|-------|--------|-------------|
| `*` | presente | Sessione corrente attiva (quella da cui si esegue il comando) |
| Line | `0 con 0` | Connessione tramite porta Console (linea 0) |
| User | — | Nessun username configurato |
| Idle | 00:00:00 | Sessione appena attiva |
| Location | — | Connessione locale, nessun IP remoto |

> [!TIP]
> Usare `show users` per verificare se un collega è connesso al router via **Telnet o SSH** (apparirebbero come linee VTY con un indirizzo IP nella colonna Location) prima di apportare modifiche alla configurazione.
