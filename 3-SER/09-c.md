...
Tutti i linguaggi — naturali e formali — hanno sintassi e semantica. La differenza sta nel **grado di precisione**:

> [!NOTE]
> **Sintassi** = le regole su *come* scrivere le istruzioni (la forma).
> **Semantica** = il *significato* di quelle istruzioni (il contenuto).
> Un linguaggio può avere sintassi rigida e semantica vaga, oppure entrambe precise — come l'Assembly.
...
quadrantChart
    ...
    C: [0.88, 0.85]
    Assembly: [0.93, 0.92]
    Logica Mat.: [0.91, 0.40]
```

> [!IMPORTANT]
> Tutti i linguaggi hanno sintassi rigida — senza regole condivise non c'è comunicazione. La vera differenza è nella semantica: quanto è preciso e non ambiguo il significato delle istruzioni?


> [!TIP]
> **Come leggere il grafico:**
> - In alto a sinistra (*Grammaticali ma ambigui*): Italiano e Inglese — hanno grammatica, ma il significato dipende dal contesto
> - In alto a destra (*Formali e precisi*): Python, C, Assembly — sintassi rigida *e* semantica univoca
> - In basso a destra (*Precisi ma informali*): Logica Matematica — semantica assoluta, ma la notazione può variare
> - In basso a sinistra (*Caotici e ambigui*): vuoto — nessun linguaggio utile vive qui

### L'ambiguità nei linguaggi naturali

> [!WARNING]
> L'ambiguità non è un difetto dei linguaggi naturali: è una caratteristica **voluta**. Gli esseri umani sfruttano il contesto, il tono, la cultura per comunicare in modo efficiente. Una macchina non può fare altrettanto.
...
In Assembly, la stessa situazione si esprime in modo completamente esplicito:

```asm
MOV EAX, [telescopio]   ; carica l'indirizzo del telescopio
MOV EBX, [uomo]         ; carica l'indirizzo dell'uomo
CMP EAX, EBX            ; confronta i due valori
```

> [!IMPORTANT]
> In Assembly non esiste ambiguità interpretativa. Ogni istruzione specifica **esattamente** quale dato usare, quale operazione eseguire, e dove mettere il risultato. Non c'è spazio per il contesto, il tono, o l'interpretazione.
...
> [!NOTE]
> L'istruzione `JE equal` (*Jump if Equal*) non è altro che la realizzazione fisica di **"se i valori sono uguali, allora salta"**. La struttura logica condizionale più elementare diventa un'operazione concreta sul processore.
